...
fn main() {
    ...
    for det in [
        ...
        let rows: Vec<_> = results
            .iter()
            .filter(|m| {
                m.detector == det && m.world.contains("abuse")
                    || m.detector == det
                        && (m.world.contains("ring") || m.world.contains("collusion") || m.world.contains("evasion"))
            })
            .collect();
        if rows.is_empty() {
            continue;
    ...
    for det in [
        ...
        if let Some(m) = results
            .iter()
            .find(|m| m.detector == det && m.world.contains("household"))
        {
            println!(
                "{det}: FP={} of {} customers (₹{:.0} friction cost)",
                m.fp,
                m.tp + m.fp + m.tn + m.fn_count,
