...
Docs: `README.md` · `docs/AI_DESIGN.md` §5 (learned now gates before gateway) · `docs/TESTING.md` · `BENCHMARK.md`
Live replay now emits `learned_insight: {model_version: lr-1.0.0-calib-0.1.0, p_hat, tau_clear 0.2345, tau_block 0.99995, band, features, contributions}` per decision — see `GET /v1/decisions/{id}` and dashboard violet card. Learned escalation now gates: `p̂·amount > ₹400` and band `review/block` escalates `ALLOW → REVIEW/BLOCK` before `DecisionMade`/gateway (BUGS #19 fixed, pinned by `learned_escalation_blocks_before_gateway`).

## Known limitations

- Data is synthetic, worlds are small (300 bg + 6–8 rings, `EVAL_SCALE=large` → 10k) — 100% precision on synthetic invites scrutiny — see `BENCHMARK.md` §1 for FP/FN costs and household checks.
